...
class Formulaire{
	...
	public function creerLabelClick($unLien, $unId, $unLabel){
		$composant = "<a href=" . $unLien . " rel=" . " stylesheet ". "id =". $unId  . ">"  . "<label>" . $unLabel . "</label>". " </a>";
		return $composant;
	}
	...
	public function creerInputSubmit($unNom, $unId, $uneValue) {
		...
		$composant .= "value='" . $uneValue . "' ";
		// Ajoutez des styles CSS pour la taille ici (par exemple, width, height, font-size, etc.)
		$composant .= "style='width: 100px; padding:20px; height: 20px; font-size: 14px;' />";
		return $composant;
	}
	
	public function creerInputBouton($unNom, $unId, $uneValue){
